...
def to_voc(webshell, wordpress):
    webshell_voc = CountVectorizer(ngram_range=(1, 1), decode_error="ignore", token_pattern=r'\b\w+\b', min_df=1)
    x1 = webshell_voc.fit_transform(webshell).toarray()
    y1 = [1] * len(x1)

    wordpress_voc = CountVectorizer(ngram_range=(1, 1), decode_error="ignore", token_pattern=r'\b\w+\b', min_df=1,
...
def main():
    ...
    print(gnb.score(x_test, y_test))  # 0.8723404255319149

    scores = cross_val_score(gnb, x, y, cv=3, scoring="accuracy")
    print(scores.mean())  # 0.8068124685771746
